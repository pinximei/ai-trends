...
def test_should_persist_diagnostic_key_info_only():
    ...
    assert should_persist_diagnostic(level="info", step="news_fetch_stats")
    assert should_persist_diagnostic(level="info", step="ingest_item")
    assert should_persist_diagnostic(level="warn", step="llm_polish_retry")
