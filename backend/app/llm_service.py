...
def polish_connector_article(
    ...
    *,
    ...
    if fk == "apps":
        stream_hint = (
            "当前条目归类为 **AI 应用** 泳道：面向产品/模型/工具/平台发布与能力更新；语气偏产品速递与开发者向；"
            "categories 优先使用「应用发布、模型更新、开源工具、Agent、推理、数据、生态」等方向的中文短标签。"
        )
    else:
        stream_hint = (
            "当前条目归类为 **AI 资讯** 泳道：面向行业动态、论文、社区与技术新闻；语气偏信息摘要与要点；"
            "categories 优先使用「大模型、论文、开源、政策与市场、安全、算力」等方向的中文短标签。"
        )
    system = (
