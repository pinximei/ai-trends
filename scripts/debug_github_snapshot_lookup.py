#!/usr/bin/env python3
from __future__ import annotations

import os
from pathlib import Path


def _load_db_url_from_systemd(unit: str = "aisoul-backend") -> None:
    marker = "AITRENDS_DATABASE_URL="
    for path in (f"/etc/systemd/system/{unit}.service", f"/lib/systemd/system/{unit}.service"):
        try:
            text = Path(path).read_text(encoding="utf-8", errors="replace")
        except OSError:
            continue
        for line in text.splitlines():
            if marker in line:
                os.environ["AITRENDS_DATABASE_URL"] = line[line.index(marker) + len(marker) :].strip()
                return


_load_db_url_from_systemd()

from sqlalchemy import desc, select

from backend.app.db import SessionLocal
from backend.app.product_models import Article, GithubTrendingSnapshot


def main() -> None:
    db = SessionLocal()
    try:
        snap = db.scalars(select(GithubTrendingSnapshot).order_by(desc(GithubTrendingSnapshot.id)).limit(1)).first()
        if not snap:
            print("no snapshot")
            return
        print("snap", snap.since, snap.period_date, snap.item_count)
        for row in (snap.items_json or [])[:5]:
            fn = str(row.get("full_name") or "")
            ext = row.get("source_external_id")
            print("item", row.get("rank"), fn, "ext", ext, "aid", row.get("article_id"))
            if ext:
                a = db.scalar(
                    select(Article)
                    .where(Article.source_external_id == str(ext)[:512])
                    .order_by(desc(Article.id))
                    .limit(1)
                )
                print("  by_ext", a.id if a else None, (a.title[:50] if a else ""))
            if fn:
                a2 = db.scalar(
                    select(Article)
                    .where(Article.source_original_url.ilike(f"%{fn}%"))
                    .order_by(desc(Article.id))
                    .limit(1)
                )
                print("  by_url", a2.id if a2 else None, (a2.title[:50] if a2 else ""))
    finally:
        db.close()


if __name__ == "__main__":
    main()
